...
def push():
    ...
    if item.get('type') == 'event':
        ...
        orig = superdesk.get_resource_service('wire_search').find_one(req=None, _id=item['guid'])
        item['_id'] = publish_item(item, orig=orig)
...
def publish_item(doc, orig=None):
    ...
    if orig and orig.get('associations'):
        for assoc in orig['associations']:
            # set missing associations to null
            doc.setdefault('associations', {}).setdefault(assoc, None)
    publish_item_signal.send(app._get_current_object(), item=doc, is_new=orig is None)
